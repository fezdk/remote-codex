...
export class FixtureCodex extends EventEmitter {
  ...
  async rpc(method,params={}){
    ...
    if(method==='model/list')return{data:[{model:'local-model',displayName:'Demo model',description:'Synthetic model for tests',supportedReasoningEfforts:[{reasoningEffort:'medium',description:'Balanced'},{reasoningEffort:'high',description:'Thorough'}],defaultReasoningEffort:'medium',hidden:false},{model:'demo-fast',displayName:'Demo fast',description:'Synthetic fast model',supportedReasoningEfforts:[{reasoningEffort:'low',description:'Fast'}],defaultReasoningEffort:'low',hidden:false},{model:'demo-hidden',displayName:'Hidden',supportedReasoningEfforts:[],hidden:true}],nextCursor:null};
    if(method==='thread/settings/update'){const thread=this.threads.find(t=>t.id===params.threadId);thread.model=params.model;if(params.effort!==undefined)thread.reasoningEffort=params.effort;this.event('thread/settings/updated',{threadId:params.threadId,threadSettings:{model:thread.model,effort:thread.reasoningEffort}});return{};}
